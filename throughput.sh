    ...
    for i in ${throughput[@]}; do
        ...
        if [ $workload_num == d ] || [ $workload_num == e ] ; then
            ...
            if [ "$CONT" == "y" ]; then
                ...
                if [ $database == dynamodb ]; then
                        echo loading $database
                        ./bin/ycsb load $database -P $db_prop -threads 10 -p recordcount=$threads_recordcount -P workloads/workload$workload_num -s > workloada_load_res.txt
                elif [ $database == cassandra-10 ]; then
                        echo loading $database
                        ./bin/ycsb load $database -p hosts=$db_prop -threads 10 -p recordcount=$threads_recordcount -P workloads/workload$workload_num -s > workloada_load_res.txt
                elif [ $database == mongodb ]; then
                        echo loading $database
                        ./bin/ycsb load $database -p mongodb.url=mongodb://$db_prop -threads 10 -p recordcount=$threads_recordcount -P workloads/workload$workload_num -s > workloada_load_res.txt
                fi
            fi
        fi
    done
